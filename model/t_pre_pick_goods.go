...
func GetPrePickGoodsJoinPrePickListByTaskId(db *gorm.DB, taskId int, number string) (err error, list []PrePickGoodsJoinPrePick) {
	err = db.Table("t_pre_pick_goods pg").
		Select("pg.id as pre_pick_goods_id,pg.*,pp.id as pre_pick_id").
		Joins("left join t_pre_pick pp on pg.pre_pick_id = pp.id").
		Where(" pp.task_id = ? and pg.number = ?", taskId, number).
		Find(&list).
		Error
...
func GetPrePickGoodsByPrePickIdAndStatusAndGoodsType(db *gorm.DB, ids []int, status int, typ int, typeParam []string) (err error, list []PrePickGoods) {
	localDb := db.Model(&PrePickGoods{}).
		Where("pre_pick_id in (?) and status = ? ", ids, status)

	if len(typeParam) > 0 {
		if typ == 2 { //按分类
			localDb.Where("goods_type in (?)", typeParam)
		} else if typ == 3 { //按商品
			localDb.Where("sku in (?)", typeParam)
		}
	}
